...
Temple::Bonfire::TextureHolder* Temple::Bonfire::TextureHolder::instance() {
...
uint32_t Temple::Bonfire::TextureHolder::addTexture(const uint8_t* pixelData, uint32_t w, uint32_t h, uint32_t ch) {
    m_textures.push_back(texture(pixelData, w, h, ch));
    return m_textures.size() - 1;
}

Temple::Base::vec4 Temple::Bonfire::TextureHolder::getPixel(uint32_t texId, float u, float v) const {
...
void Temple::Bonfire::TextureHolder::saveTexture(const char* path, int texId) {
