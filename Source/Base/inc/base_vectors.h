...
namespace Base
{
  ...
  struct vec3_t {
    ...
    {
      float l = sqrt((x - v.x) * (x - v.x) + (y - v.y) * (y - v.y) + (z - v.z) * (z - v.z));
      return l < Base::eps;
    }
    inline const bool
    operator!=(const vec3_t& v) const
  ...
  struct vec4_t {
    ...
    {
      float l = sqrt((x - v.x) * (x - v.x) + (y - v.y) * (y - v.y) + (z - v.z) * (z - v.z) + (w - v.w) * (w - v.w));
      return l < Base::eps;
    }
    inline const bool
    operator!=(const vec4_t& v) const
    ...
    {
      return vec4_t {y * v.z - z * v.y, z * v.x - x * v.z, x * v.y - y * v.x, 0.0f};
    }
  };
  inline vec4_t
  operator-(const vec4_t& v)
