...
sisyphus_application_init(void* data)
{
  ...
  s_render_context.set_vertex_shader(
    [](
      const Base::vec4_t& inp, Base::vec4_t& out, std::vector<uint8_t>& per_vertex_out, const uint8_t* per_vertex_data,
      const std::vector<uint8_t>& builtins, const std::vector<uint8_t>& descriptor_set)
    {
  ...
  for (int i = 0; i < obj_file->faces.size(); i++)
  {
    ...
    for (int j = 0; j < 3; j++)
    {
      ...
      Base::vec4_t pos {obj_file->coord[vert_idx].x, obj_file->coord[vert_idx].y, obj_file->coord[vert_idx].z, 1.0f};
...
sisyphus_application_update(void* data)
{
  ...
  s_render_context.draw_triangles(
    s_abc, s_abc_triangle_indices, reinterpret_cast<const uint8_t*>(s_abc_triangle_attribs.data()),
    s_vertex_input_format, s_vertex_output_format);
  s_render_context.draw_triangles(
    s_model_verts, s_model_inds, reinterpret_cast<const uint8_t*>(s_model_vertex_attribs.data()), s_vertex_input_format,
    s_vertex_output_format);
#if TRIANGLE_LINE
  s_render_context.draw_lines(
    s_abc, s_abc_line_indices, reinterpret_cast<const uint8_t*>(s_abc_triangle_attribs.data()), s_vertex_input_format,
    s_vertex_output_format);
#endif
}
