...
extern "C" 
{
  void temple_application_init(void* data)
  {
        ...
        const std::vector<uint8_t>& descriptor_set) -> Base::vec4_t
    {
      ...
      Base::vec4_t tex_color =
        Bonfire::TextureHolder::instance()->get_pixel(
          0,
          p_tex->u,
          p_tex->v);
      // illumination
    ...
    std::shared_ptr<Barn::ObjFile> obj_file =
      Barn::read_obj_model_file(obj_path);
    ...
    for (int i = 0; i < obj_file->faces.size(); i++)
    {
      ...
      for (int j = 0; j < 3; j++)
      {
        ...
        Base::append_data(
          s_model_vertex_attribs,
          obj_file->normal[normalIdx]);
      }
    }
  }

  void temple_application_update(void* data)
  {
    ...
    Base::mat4_t translation_matrix =
      Base::mat4_t::get_identity_matrix();
    ...
    s_render_context.draw_triangles(
      ...
      reinterpret_cast<const uint8_t*>(s_abc_triangle_attribs.data()), //reinterpret_cast<const uint8_t*>(g_modelVertAttribs.data()),
  ...
  void temple_application_set_log_function(void(*func)(const char* msg, unsigned int msg_size)) 
  {
    s_render_context.set_log_func(func);
  }

  void temple_application_close()
  {
    // s_render_context.destroy();
  }

  void temple_application_get_frame(void* p_data, unsigned int data_size)
  {
